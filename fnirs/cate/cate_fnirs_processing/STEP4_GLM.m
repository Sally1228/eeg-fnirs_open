%% STEP4_GLM.m
% GLM semantic fluency task (single condition)

%% Assumptions & Notes
% - stim(1) corresponds to the semantic fluency task (marker=1), repeated blocks.
% - If stim duration is missing/<=0, default to stimDurationSec (60s).
% - Rest periods are implicit in the onsets/durations and not modeled explicitly.
...
taskList = {'cate'};        % used for input/output naming
...
% Stim index (in preproc MAT): 1=task (marker=1)
stimIndexTask = 1;
...
includeBlockRegressor = false; % single-condition block regressor is collinear; keep off
blockTrials = 1;
blockDurationSec = 60;      % block持续时间（秒）
...
fprintf('[INFO] Assumptions: stim(1)=task (marker=1), defaultDur=%.1fs, blockReg=%d\n', ...
    stimDurationSec, includeBlockRegressor);
fprintf('[INFO] Note: rest periods are implicit in stim onsets/durations.\n');
...
for si = 1:numel(subjList)
    ...
    for ti = 1:numel(taskList)
        ...
        stimIndexTaskUse = stimIndexTask;

        if nStim < stimIndexTaskUse
            error('stim has only %d entries; need stim(%d).', nStim, stimIndexTaskUse);
        end
        ...
        % Task regressor from stim(1) events and HRF convolution
        stimTask = stim(stimIndexTaskUse).data;
        if isempty(stimTask)
            error('stim(%d) is empty.', stimIndexTaskUse);
        end
        uTask = zeros(nT,1);
        for ei = 1:size(stimTask,1)
            onset = stimTask(ei,1);
            if size(stimTask,2) >= 2 && stimTask(ei,2) > 0
                dur = stimTask(ei,2);
            else
                dur = stimDurationSec;
            end
            if size(stimTask,2) >= 3
                amp = stimTask(ei,3);
            else
                amp = 1;
            end
        ...
        if saveHrfFigure
            ...
            subplot(2,1,2);
            plot(t, xTask, 'Color', colorHbO);
            xlabel('Time (s)');
            ylabel('Regressor (a.u.)');
            title('Convolved regressor: task', 'Interpreter','none');
        ...
        % Block duration fallback (from events) 估计 block 的持续时间，用于 block 回归项
        dataMerged = stimTask;
        ...
        if includeBlockRegressor
            ...
            if ~isempty(blockIdx)
                ...
                % build from event onsets if block_onset is missing
        ...
        % no contra-ROI logic for single-condition task
        ...
        save(outMat, 'beta', 'seReg', 'tReg', 'pReg', ...
        ...
        % no contrast figures for single-condition task
