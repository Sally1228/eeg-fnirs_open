...
for i = 1:length(SUB)
    ...
    EEG  = pop_creabasiceventlist(EEG, 'AlphanumericCleaning', 'on', 'BoundaryNumeric', { -99 }, 'BoundaryString', { 'boundary' }, ...
        'Eventlist', fullfile(Subject_Path, [SUB{i} '_motor_Eventlist.txt'])); 
    ...
    %Assign events to bins with Binlister; an individual trial may be assigned to more than one bin (bin assignments can be reviewed in each subject's motor_Eventlist_Bins.txt file)
    EEG  = pop_binlister(EEG, 'BDF', fullfile(input_dir, 'BDF_motor.txt'), ...
        'ExportEL', fullfile(Subject_Path, [SUB{i} '_motor_Eventlist_Bins.txt']), 'IndexEL', 1, 'SendEL2', 'EEG&Text', 'UpdateEEG', 'on', 'Voutput', 'EEG');
    ...
    %Epoch the EEG 握拳30s，休息15s
    %基线-2000ms 0-2s可能有准备电位，至40000ms[40s]，先不做基线矫正
    EEG = pop_epochbin(EEG, [-2000 40000], 'none');
