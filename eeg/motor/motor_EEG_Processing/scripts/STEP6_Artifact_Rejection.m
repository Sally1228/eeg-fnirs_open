...
% 需要文档：Interpolate_Channels_motor、AR_Parameters_for_SVT_CRAP_motor、AR_Parameters_for_MW_CRAP_motor
...
[ndata1, text1, alldata1] = xlsread(fullfile(input_dir, 'Interpolate_Channels_motor.xlsx'));
...
[ndata2, text2, alldata2] = xlsread(fullfile(input_dir, 'AR_Parameters_for_SVT_CRAP_motor.xlsx'));
...
[ndata3, text3, alldata3] = xlsread(fullfile(input_dir, 'AR_Parameters_for_MW_CRAP_motor.xlsx'));
...
for i = 1:length(SUB)
    ...
    %Interpolate channel(s) specified in Excel file Interpolate_Channels_motor.xls; any channels without channel locations (e.g., the eye channels) should not be included in the interpolation process and are listed in ignored channels
