...
% 需要文档：Interpolate_Channels_recog、AR_Parameters_for_SVT_CRAP_recog、AR_Parameters_for_MW_CRAP_recog
%Operates on individual subject data
%Uses the output from Script #6: Elist_Bin_Epoch.m
...
fprintf('[%s] STEP7 Artifact rejection\n', datestr(now, 'yyyy-mm-dd HH:MM:SS'));
...
[ndata1, text1, alldata1] = xlsread(fullfile(input_dir, 'Interpolate_Channels_recog.xlsx'));
...
[ndata2, text2, alldata2] = xlsread(fullfile(input_dir, 'AR_Parameters_for_SVT_CRAP_recog.xlsx'));
...
[ndata3, text3, alldata3] = xlsread(fullfile(input_dir, 'AR_Parameters_for_MW_CRAP_recog.xlsx'));
%目前数据中没有记录眼动
...
% [ndata4, text4, alldata4] = xlsread([Current_File_Path filesep 'AR_Parameters_for_MW_Blinks_P3']);
...
%[ndata5 text5, alldata5] = xlsread([Current_File_Path filesep 'AR_Parameters_for_SL_HEOG_Stim_Pres_P3']);
...
%[ndata6 text6, alldata6] = xlsread([Current_File_Path filesep 'AR_Parameters_for_SL_HEOG_P3']);
...
for i = 1:length(SUB)
    ...
    EEG = pop_loadset('filename', [SUB{i} '_task-' task '_eeg_ds_reref_hpfilt_ica_corr_marker_elist_bins_epoch.set'], 'filepath', Subject_Path);
    [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 1, 'setname', [SUB{i} '_task-' task '_eeg_ds_reref_hpfilt_ica_corr_marker_elist_bins_epoch'], 'overwrite','on', 'gui', 'off'); 

    %Interpolate channel(s) specified in Excel file Interpolate_Channels_recog.xls; any channels without channel locations (e.g., the eye channels) should not be included in the interpolation process and are listed in ignored channels
    ...
    for j = 1:DimensionsOfFile1(1);
        if isequal(SUB{i},num2str(alldata1{j,1}));
           ...
           [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 2, 'setname', [SUB{i} '_task-' task '_eeg_ds_reref_hpfilt_ica_corr_marker_elist_bins_epoch_interp'], ...
               'savenew', fullfile(Subject_Path, [SUB{i} '_task-' task '_eeg_ds_reref_hpfilt_ica_corr_marker_elist_bins_epoch_interp.set']), 'overwrite','on', 'gui', 'off'); 
        end
    end
    ...
    [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 3, 'setname', [SUB{i} '_task-' task '_eeg_ds_reref_hpfilt_ica_corr_marker_elist_bins_epoch_interp_SVT'], 'overwrite','on','gui', 'off'); 
    ...
    for j = 1:DimensionsOfFile3(1)
        if isequal(SUB{i},num2str(alldata3{j,1}));
            if isequal(alldata3{j,2}, 'default')
                Channels = 1:62; %左右电极，如果耳电极波动大，也会影响结果
            else
    ...
    [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 4, 'setname', [SUB{i} '_task-' task '_eeg_ds_reref_hpfilt_ica_corr_marker_elist_bins_epoch_interp_SVT_MW1'], ...
        'savenew', fullfile(Subject_Path, [SUB{i} '_task-' task '_eeg_ds_reref_hpfilt_ica_corr_marker_elist_bins_epoch_interp_ar.set']), 'overwrite','on', 'gui', 'off'); 
