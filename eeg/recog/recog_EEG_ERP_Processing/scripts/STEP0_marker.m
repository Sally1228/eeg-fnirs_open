...
% script_dir = '/Users/zhaifeifei/Desktop/eeg_fnirs/eeg/recog/recog_EEG_ERP_Processing/scripts'
...
fprintf('[%s] STEP0 Convert trigger channels\n', datestr(now, 'yyyy-mm-dd HH:MM:SS'));

% List of subjects to process, based on the name of the folder that contains that subject's data
SUB = { 'sub-002', 'sub-003', 'sub-004', 'sub-005', 'sub-008', 'sub-009', 'sub-010' };

% Trigger channel indices in the EEG data 
trigChans = [38 39 40 41];
...
for i = 1:numel(SUB)
    sid = SUB{i};
    subFolder = fullfile(data_dir, sid);

    baseName = sprintf('%s_task-recog_eeg', sid);
    ...
    %% Save .set
    EEG = pop_saveset(EEG, 'filename', outFile, 'filepath', set_root);
    fprintf('Saved: %s\n', fullfile(set_root, outFile));
end

marker_files_folder = fullfile(project_dir,'outputs');
if ~exist(marker_files_folder, 'dir')
    mkdir(marker_files_folder);
end

if export_marker_tables
    writecell(all_marker_rows, fullfile(marker_files_folder, 'recog_marker_events.csv'));
    writecell(summary_rows, fullfile(marker_files_folder, 'recog_marker_counts.csv'));
end
