...
%Uses the output from Script #7: Artifact_Rejection.m
...
for i = 1:length(SUB)
    ...
    EEG = pop_loadset('filename', [SUB{i} '_task-' task '_eeg_ds_reref_hpfilt_ica_corr_marker_elist_bins_epoch_interp_ar.set'], 'filepath', Subject_Path);
    ...
    ERP = pop_savemyerp(ERP, 'erpname', [SUB{i} '_recog_erp_ar'], 'filename', fullfile(Subject_Path, [SUB{i} '_recog_erp_ar.erp']));
    ...
    ERP = pop_savemyerp(ERP, 'erpname', [SUB{i} '_recog_erp_ar_lpfilt'], 'filename', fullfile(Subject_Path, [SUB{i} '_recog_erp_ar_lpfilt.erp']));
    ...
    fid = fopen(fullfile(data_dir, SUB{i}, [SUB{i} '_AR_Percentages_recog.csv']), 'w');
...
for i = 1:length(SUB)
    ...
    ERP = pop_loaderp('filename', [SUB{i} '_recog_erp_ar_lpfilt.erp'], 'filepath', Subject_Path);                                                                                                                                                                                                                                                                                                                                       

    %Create ERP difference waveforms between conditions
    ERP = pop_binoperator(ERP, fullfile(input_dir, 'recog_Diff_Wave.txt'));
    ERP = pop_savemyerp(ERP, 'erpname', [SUB{i} '_recog_erp_ar_diff_waves'], 'filename', fullfile(Subject_Path, [SUB{i} '_recog_erp_ar_diff_waves.erp']));
    ...
    ERP = pop_filterp( ERP,  1:62 , 'Cutoff',  20, 'Design', 'butter', 'Filter', 'lowpass', 'Order',  8 );
    ERP = pop_savemyerp(ERP, 'erpname', [SUB{i} '_recog_erp_ar_diff_waves_lpfilt'], 'filename', fullfile(Subject_Path, [SUB{i} '_recog_erp_ar_diff_waves_lpfilt.erp']));
