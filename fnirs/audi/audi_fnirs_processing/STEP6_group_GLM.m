%% STEP4.m
% Group-level analysis from subject ROI GLM tables (STEP3_mixedGLM output)
%
% Assumptions & Notes:
...
if isempty(allTables)
    error('No ROI tables loaded. Check subject folders and STEP3_mixedGLM outputs.');
end
...
if any(termCol == plotTerm)
    for ti = 1:numel(tasks)
        ...
        title(sprintf('%s group ROI contrast', taskName), 'Interpreter','none');
        figPath = fullfile(figDir, sprintf('group_task-%s_step-glm_roi_contrast.png', taskName));
        saveas(fig1, figPath);
        close(fig1);
    end
end
