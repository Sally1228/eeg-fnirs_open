...
% Contralateral ROI mapping (left hand -> right central, right hand -> left central)
useContraRoiOnly = true;
contraRoiForLeft = 'RightCentral';
contraRoiForRight = 'LeftCentral';
includeRightLeftContrast = false; % set true if you want the combined contrast

% Event parameters (fallbacks)
stimDurationSec = 30;

% Mixed design options
includeBlockRegressor = false; %左右手分别建模时，整体block回归与left/right高度共线，默认关闭
blockTrials = 1;
blockDurationSec = 30;      % block持续时间（秒）
...
% Drift terms
driftModel = 'dct';         % 'none' | 'linear' | 'poly' | 'dct'   漂移建模方式；dct 表示用 DCT 基（相当于高通/去低频）
...
% GLM method 推荐用ar-irls
glmMethod = 'ar-irls';      % 'ar-irls' | 'ols'

% AR-IRLS settings (Barker et al.)
arIrlsOrderSec = 1         % 用1-2不能收敛 AR model order in seconds (Pmax = round(arIrlsOrderSec/dt)) AR-IRLS 的自回归模型阶数对应的时间尺度（秒），脚本里用Pmax=round(arIrlsOrderSec/dt) 转成点数
...
fprintf('[INFO] Assumptions: stim(1)=left, stim(2)=right, blockReg=%d\n', includeBlockRegressor);
if useContraRoiOnly
    fprintf('[INFO] Contra-ROI: left->%s, right->%s\n', contraRoiForLeft, contraRoiForRight);
end
...
for si = 1:numel(subjList)
    ...
    for ti = 1:numel(taskList)
        ...
        stimIndexLeftUse = stimIndexLeft;
        stimIndexRightUse = stimIndexRight;

        if nStim < max(stimIndexLeftUse, stimIndexRightUse)
            error('stim has only %d entries; need stim(%d) and stim(%d).', ...
                nStim, stimIndexLeftUse, stimIndexRightUse);
        end
        ...
        % Left regressor 构造左手握拳回归量。先从 stim(1) 取出所有 left
        % 的事件信息，用 uLeft 在时间轴上画出一个"方波事件序列”，然后用 HRF 做卷积：xLeft = conv(uLeft, hrf)，
        % 把神经事件转成预期的血流响应。最后一行 xLeft = xLeft(1:nT) 是把卷积结果裁剪回原始长度。
        stimLeft = stim(stimIndexLeftUse).data;
        if isempty(stimLeft)
            error('stim(%d) is empty.', stimIndexLeftUse);
        end
        uLeft = zeros(nT,1);
        for ei = 1:size(stimLeft,1)
            onset = stimLeft(ei,1);
            if size(stimLeft,2) >= 2 && stimLeft(ei,2) > 0
                dur = stimLeft(ei,2);
            else
                dur = stimDurationSec;
            end
            if size(stimLeft,2) >= 3
                amp = stimLeft(ei,3);
            else
                amp = 1;
            end
        ...
        % Right regressor 构造右手握拳回归量
        stimRight = stim(stimIndexRightUse).data;
        if isempty(stimRight)
            error('stim(%d) is empty.', stimIndexRightUse);
        end
        uRight = zeros(nT,1);
        for ei = 1:size(stimRight,1)
            onset = stimRight(ei,1);
            if size(stimRight,2) >= 2 && stimRight(ei,2) > 0
                dur = stimRight(ei,2);
            else
                dur = stimDurationSec;
            end
            if size(stimRight,2) >= 3
                amp = stimRight(ei,3);
            else
                amp = 1;
            end
        ...
        if saveHrfFigure
            ...
            subplot(2,1,2);
            plot(t, xLeft, 'Color', colorHbO); hold on;
            plot(t, xRight, 'Color', colorHbR);
            xlabel('Time (s)');
            ylabel('Regressor (a.u.)');
            title('Convolved regressors: left vs right', 'Interpreter','none');
        ...
        %% ===================== [6] DESIGN MATRIX =====================
        %构建 GLM 的设计矩阵 X：先放入两个事件回归量：xLeft 和 xRight。
        ...
        if useContraRoiOnly
            ...
            if ~any(strcmp(roiNames, contraRoiForLeft)) || ~any(strcmp(roiNames, contraRoiForRight))
                error('Contra ROI names not found in roiDefs: left=%s right=%s', ...
                    contraRoiForLeft, contraRoiForRight);
            end
        end
        ...
        % Contrast: right - left 做右手 vs 左手的对比
        if includeRightLeftContrast
            c = zeros(p,1);
            c(strcmp(regNames, 'right')) = 1;
            c(strcmp(regNames, 'left')) = -1;
        end
        ...
        for mi = 1:nMeas
            ...
            for ri = 1:numel(regNames)
                ...
                if useContraRoiOnly
                    if strcmp(termName, 'left') && roiLabel(mi) ~= contraLeftStr
                        continue;
                    elseif strcmp(termName, 'right') && roiLabel(mi) ~= contraRightStr
                        continue;
                    end
                end
            ...
            if includeRightLeftContrast
                rowCell(end+1,:) = {subName, taskName, mi, src(mi), det(mi), char(chrom(mi)), char(roiLabel(mi)), ...
                    'right-left', betaC(mi), seC(mi), tC(mi), pC(mi)}; %#ok<AGROW>
            end
        end
            ...
            for ri = 1:numel(roiList)
                ...
                for ci = 1:numel(chromList)
                    ...
                    for ti2 = 1:numel(termList)
                        ...
                        if useContraRoiOnly
                            if strcmp(termName, 'left') && ~strcmp(roiName, contraRoiForLeft)
                                continue;
                            elseif strcmp(termName, 'right') && ~strcmp(roiName, contraRoiForRight)
                                continue;
                            end
                        end
                        if strcmp(termName, 'right-left')
                            vals = betaC(chanIdx);
                        else
