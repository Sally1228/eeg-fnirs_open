...
fprintf('[%s] STEP10 Grand average ERPs\n', datestr(now, 'yyyy-mm-dd HH:MM:SS'));

%List of subjects to process, based on the name of the folder that contains that subject's data
%SUB = { '002', '003', '004', '005', '006', '008', '009', '010'};   
...
ERPset_list = fullfile(ga_output_dir, 'GA_recog_erp_ar_diff_waves.txt');
...
ERP = pop_savemyerp(ERP, 'erpname', 'GA_recog_erp_ar_diff_waves', 'filename', 'GA_recog_erp_ar_diff_waves.erp', 'filepath', ga_output_dir, 'Warning', 'off');
...
ERPset_list = fullfile(ga_output_dir, 'GA_recog_erp_ar_diff_waves_lpfilt.txt');
fid = fopen(ERPset_list, 'w');
    for i = 1:length(SUB)
        Subject_Path = fullfile(data_dir, SUB{i});
...
%Create a grand average ERP waveform
ERP = pop_gaverager( ERPset_list , 'ExcludeNullBin', 'on', 'SEM', 'on' );
ERP = pop_savemyerp(ERP, 'erpname', 'GA_recog_erp_ar_diff_waves_lpfilt', 'filename', 'GA_recog_erp_ar_diff_waves_lpfilt.erp', 'filepath', ga_output_dir, 'Warning', 'off');
