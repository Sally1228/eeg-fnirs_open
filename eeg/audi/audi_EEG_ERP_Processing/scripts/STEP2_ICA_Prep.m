...
if ~strcmp(script_parent, 'scripts')
...
for i = 1:length(SUB)
    ...
    %Remove segments of EEG during the break periods in between trial blocks (defined as 2.5 seconds or longer in between successive stimulus event codes)
    %删除休息段，有效的marker是1和2,间隔>2.5s，前后各预留2s
    EEG  = pop_erplabDeleteTimeSegments( EEG , 'displayEEG', 0, 'endEventcodeBufferMS',  2000, 'ignoreUseEventcodes', [1,2], 'ignoreUseType', 'Use', 'startEventcodeBufferMS',  2000, 'timeThresholdMS',  2500 );
    [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 2, 'setname', [SUB{i} '_shifted_ds_reref_ucbip_hpfilt_ica_pre1'], ...
        'savenew', fullfile(Subject_Path, [SUB{i} '_task-' task '_eeg_ds_reref_hpfilt_ica_pre1.set']), 'overwrite', 'on', 'gui', 'off'); 
    ...
    [ndata, text, alldata] = xlsread(fullfile(Current_File_Path, 'ICA_Prep_Values_audi.xlsx')); 
    ...
    [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 3, 'setname', [SUB{i} '_shifted_ds_reref_ucbip_hpfilt_ica_pre2'], ...
