...
for i = 1:length(SUB)
    ...
    EEG  = pop_creabasiceventlist(EEG, 'AlphanumericCleaning', 'on', 'BoundaryNumeric', { -99 }, 'BoundaryString', { 'boundary' }, ...
        'Eventlist', fullfile(Subject_Path, [SUB{i} '_audi_Eventlist.txt'])); 
    ...
    %Assign events to bins with Binlister; an individual trial may be assigned to more than one bin (bin assignments can be reviewed in each subject's audi_Eventlist_Bins.txt file)
    EEG  = pop_binlister(EEG, 'BDF', fullfile(input_dir, 'BDF_audi.txt'), ...
        'ExportEL', fullfile(Subject_Path, [SUB{i} '_audi_Eventlist_Bins.txt']), 'IndexEL', 1, 'SendEL2', 'EEG&Text', 'UpdateEEG', 'on', 'Voutput', 'EEG');
    ...
    %先改成-200 到1000ms看看
    EEG = pop_epochbin( EEG , [-200.0  1000.0],  [-200.0  0.0]);
