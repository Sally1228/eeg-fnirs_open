...
% - Tables include "condition" (task) and ROI (LeftFrontal/LeftTemporal/RightFrontal/RightTemporal).
...
% Plot options
condOrder = {'task'};
roiOrder = {'LeftFrontal','LeftTemporal','RightFrontal','RightTemporal'};
chromOrder = {'HbO','HbR'};
...
fprintf('[INFO] Assumptions: condition=task, ROI=LeftFrontal/LeftTemporal/RightFrontal/RightTemporal\n');
...
roiFromData = cellstr(unique(allT.roi));
roiListUse = roiOrder(ismember(roiOrder, roiFromData));
extraRoi = setdiff(roiFromData, roiOrder, 'stable');
roiListUse = [roiListUse, extraRoi];
if isempty(roiListUse)
    roiListUse = roiFromData;
end
...
for ti = 1:numel(tasks)
    ...
    for ri = 1:numel(roiListUse)
        roiName = string(roiListUse{ri});
        epochTimeBase = [];
        hboAll = [];
        hbrAll = [];
        ...
        for si = 1:numel(subjList)
            ...
            if ~isfield(S, 'results') || ~isfield(S.results, 'rois')
                continue;
            end
            res = S.results;
            if isempty(res.rois)
            ...
            roiNamesSub = string({res.rois.name});
            idx = find(roiNamesSub == roiName, 1);
            if isempty(idx)
                continue;
            end
            rRes = res.rois(idx);
            ...
            if isempty(epochTimeBase)
                epochTimeBase = rRes.epochTime(:);
            end

            hboVec = rRes.hboAvg(:);
            hbrVec = rRes.hbrAvg(:);

            if numel(rRes.epochTime) ~= numel(epochTimeBase) || ...
                    max(abs(rRes.epochTime(:) - epochTimeBase)) > 1e-6
                hboVec = interp1(rRes.epochTime(:), hboVec, epochTimeBase, 'linear', NaN);
                hbrVec = interp1(rRes.epochTime(:), hbrVec, epochTimeBase, 'linear', NaN);
            end
        ...
        roiCount = roiCount + 1;
        groupTC(ti).roi(roiCount).name = char(roiName);
        groupTC(ti).roi(roiCount).epochTime = epochTimeBase;
        groupTC(ti).roi(roiCount).hboMean = hboMean;
        groupTC(ti).roi(roiCount).hbrMean = hbrMean;
        groupTC(ti).roi(roiCount).hboSe = hboSe;
        groupTC(ti).roi(roiCount).hbrSe = hbrSe;
        groupTC(ti).roi(roiCount).nSubHbo = nSubHbo;
        groupTC(ti).roi(roiCount).nSubHbr = nSubHbr;
    end
    ...
    for ri = 1:nRoi
        ...
        xline(0, '--k');
        ylabel(sprintf('%s | %s', taskName, groupTC(ti).roi(ri).name), 'Interpreter','none');
        if ri == 1
